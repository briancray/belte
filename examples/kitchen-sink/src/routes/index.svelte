...
<section class="mt-6 rounded-lg border border-slate-200 bg-white p-5">
    ...
    <p class="mt-1 text-sm text-slate-600">
        Cookie-based session. <code class="font-mono">routes/dashboard.ts</code> is colocated with a
        page and returns <code class="font-mono">&#123; redirect: '/login' &#125;</code> when no
        session is present.
    </p>
    <div class="mt-3 flex items-center gap-3 text-sm">
        {#if data.user}
            <span class="text-slate-600">Signed in as <strong>{data.user}</strong>.</span>
        ...
        {:else}
            ...
            <a class="text-slate-900 underline" href="/dashboard">Try /dashboard (will redirect)</a>
        {/if}
    </div>
</section>
