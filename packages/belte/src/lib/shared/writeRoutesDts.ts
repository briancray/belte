...
function paramsForRoute(routeUrl: string): Record<string, 'string'> {
    ...
    for (const segment of routeUrl.split('/')) {
        if (segment.startsWith('[...') && segment.endsWith(']')) {
            params[segment.slice(4, -1)] = 'string'
            continue
        }
        if (segment.startsWith('[') && segment.endsWith(']')) {
            params[segment.slice(1, -1)] = 'string'
        }
    }
    return params
}
...
page file in the project. AppState picks this up as a discriminated union
keyed on `route`, so `if (nav.route === '/media/[id]') nav.params.id` is
typed automatically without consumers writing route types by hand.
