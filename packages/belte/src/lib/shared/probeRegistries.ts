...
export function probeRegistries<Args, Return>(
    ...
    return (
        ...
        streams?.[field] === true ||
        /* A parked durable write has no value yet → pending only (never refreshing). The
           outbox prober is registered browser-side; absent on the server, the term is false. */
