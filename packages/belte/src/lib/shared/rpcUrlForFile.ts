...
is one endpoint at `/rpc/<file path>`, dropping the `.ts` extension. $rpc
URLs are flat function-call endpoints (args go in query or body), so
...
export function rpcUrlForFile(relPath: string): string {
    ...
    for (const segment of segments) {
        if (segment.startsWith('[')) {
            throw new Error(
                `[belte] src/rpc/${relPath} has a dynamic segment '${segment}' — $rpc URLs are flat; pass identifiers via args, not the path`,
            )
        }
    }
