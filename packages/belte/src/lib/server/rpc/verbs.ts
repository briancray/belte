...
/*
Declares a remote handler inside an `$rpc/**` module. Each file
...
path under `src/server/rpc/`, then rewrites this call to bind all three into
...
For broadcast/pub-sub use the `belte/sockets` primitive — every
multiplexed ws fan-out lives under `src/server/sockets/` now. For per-call
iteration over an SSE/JSONL response, use `fn.stream(args)` on the
returned RemoteFunction.
