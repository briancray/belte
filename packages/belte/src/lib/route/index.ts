...
/*
Declares a remote handler inside an `$route/**` module. Each file contains
...
`src/route/`, then rewrites this call to bind all three into the runtime
...
export const SOCKET: SocketHelper = (_fn) => {
    throw new Error(
        '[belte] `SOCKET` was called outside an $route module — verb helpers are only valid as the value of `export const <filename> = ...` inside a file under src/route/',
    )
}
